...
export default function AllCourseViewDashboard() {
  ...
  const enrollInCourseEventHandler = async (courseId: string) => {
    ...
    // Re-fetch the courses or the enrolled courses to update the UI
    fetchCoursesEventHandler();
  };
    ...
    <div id="wd-dashboard" className="p-4" >
      ...
      <div>
        <p>
          ...
          >
            <h4>Currently Enrolled Courses ({userEnrolledCourses.length})</h4>
          </button>
        </p>
        <div className="collapse" style={{   border: "none" }} id="collapseExample2">
      ...
      <br />
      <br />
      <h2 id="wd-dashboard-published">Available Published Courses</h2><hr />
      <br />
      ...
      <div id="wd-dashboard-courses" className="row">
        <div className="row row-cols-1 row-cols-md-5 g-4">
          {courses
            .filter((course: any) => !currentUser.enrolledCourses.includes(course._id)) // Filter out enrolled courses
            .map((course: any) => (
              <div key={course._id} id="wd-dashboard-course" className="col" style={{ width: "300px" }}>
                <div className="card">
                  <Link to={`/Kanbas/Courses/${course.number}/Home`} className="text-decoration-none">
